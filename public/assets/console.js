(() => {
 ...
 const templateLabels={feiyue:'Feiyue / ReelShort',dptv:'DPTV / MinuteDrama',quest:'Quest / Pasion TV',aivideo:'AI Video / 阿语短视频',ggtv:'GGTV',appstore:'Google Play / 应用详情',ultraplay:'Ultra Play',yacinetv:'Yacine TV',dptvplus:'DPTV / 电视介绍',fizzio:'Fizzio Live',kyss:'KYSS',sparkle:'Sparkle',newf:'NEW-F',soccerqueens:'Soccer Queens',cinema:'Netflix Gratis / 影视目录',noxxtv:'NOXX TV',smarttrade:'SmartTrade',stockvault:'StockVault',kuaibo:'KuaiBo',tikapk:'Tik APK',jiaoyou:'交友',cyberheart:'Cyber Heart Show',promotions:'Promotions',india:'印度',apuestamx:'ApuestaMX',loveapp:'LoveApp',qiyou:'qiyou',stripchat:'STRIPCHAT',pornparadise:'Porm Paradise',pornhub:'Pornhub',chatee:'Chatee Live Now',cosplay:'Cosplay',lustylive:'Lusty Live',nightplay:'NightPlay',bokep:'Website Bokep',xhamster:'XHAMSTER',mylive:'MyLive'};
 const templateBrands={feiyue:'ReelShort',dptv:'DPTV',quest:'Pasion TV',aivideo:'AI Video',ggtv:'GGTV',appstore:'Cast',ultraplay:'Ultra Play',yacinetv:'Yacine TV',dptvplus:'DPTV',fizzio:'Fizzio',kyss:'KYSS',sparkle:'Sparkle',newf:'NEW-F',soccerqueens:'Soccer Queens',cinema:'Netflix Gratis',noxxtv:'NOXX',smarttrade:'SmartTrade',stockvault:'StockVault',kuaibo:'KuaiBo',tikapk:'Tik APK',jiaoyou:'交友',cyberheart:'Cyber Heart Show',promotions:'VELOCITY GAMING',india:'印度',apuestamx:'ApuestaMX',loveapp:'LoveApp',qiyou:'qiyou',stripchat:'STRIPCHAT',pornparadise:'Porm Paradise',pornhub:'Pornhub',chatee:'Chatee',cosplay:'Cosplay',lustylive:'Lusty Live',nightplay:'NightPlay',bokep:'Website Bokep',xhamster:'XHAMSTER',mylive:'MyLive'};
 ...
 function renderRows(){const q=state.search.trim().toLowerCase(),rows=state.projects.filter(p=>(state.filter==='all'||p.status===state.filter)&&[p.name,p.slug,p.username].some(v=>v.toLowerCase().includes(q)));$('#project-rows').innerHTML=rows.map((p,i)=>`<tr><td><div class="d-flex align-items-center gap-3"><div class="avatar bg-primary-lt tone-${i%4}">${escape((p.app_name||p.name).slice(0,1).toUpperCase())}</div><div><strong>${escape(p.name)}</strong><small class="font-monospace d-block text-secondary">/p/${p.slug}</small></div></div></td><td><code>${escape(p.username)}</code><div><button class="btn btn-link btn-sm" data-action="credentials" data-slug="${p.slug}">查看密码</button>${p.credentialsAvailable?'':'<small class="text-secondary">旧版账号</small>'}</div></td><td>${status(p)}</td><td><span class="badge bg-blue-lt">${escape(templateName(p.template))}</span></td><td>${n(p.visits)} <span class="font-monospace">/ ${n(p.conversions)}</span></td><td>${date(p.created_at)}</td><td><button class="btn btn-link btn-sm" data-action="manage" data-slug="${p.slug}">管理</button><button class="btn btn-link btn-sm" data-action="copy-delivery" data-slug="${p.slug}">复制交付</button><button class="btn btn-link btn-sm text-secondary" data-action="copy-url" data-slug="${p.slug}">复制地址</button><button class="btn btn-link btn-sm text-danger" data-action="delete-project" data-slug="${p.slug}">删除</button></td></tr>`).join('')||'<tr><td colspan="7"><div class="empty text-secondary py-4"><span>▦</span><b>这里还没有匹配的项目</b><p>试试其他关键词，或点击右上角开通一个新项目。</p></div></td></tr>';$('#project-count').textContent='显示 '+rows.length+' 个项目 · 共 '+state.projects.length+' 个';}
  ...
  return `<section class="card"><div class="card-header"><h2 class="card-title">落地页模板</h2><span class="badge bg-primary-lt">${p.allowedTemplates.length} / ${templateIds.length} 款对客户开放</span></div><div class="card-body"><form id="template-permissions-form">
   ...
   <div class="template-grid">${templateIds.map(t=>`<article class="card mb-0 ${p.template===t?'border-primary':''}">
    <div class="d-flex align-items-center justify-content-between flex-wrap gap-2 p-3 border-bottom">
     ...
     ${p.template===t?'<span class="badge bg-primary text-white">使用中</span>':''}
    </div>
    <button type="button" class="btn p-0 border-0 rounded-0 d-block w-100 text-start text-wrap" data-action="preview" data-template="${t}" aria-label="预览：${escape(templateName(t))}">
     <span class="card-header bg-primary-lt justify-content-center py-5 ${t}"><strong>${escape(templateBrands[t])}</strong></span>
 ...
 function syncTemplatePermissions(){
  ...
  checks.forEach(input=>{
   const saved=p.allowedTemplates.includes(input.value),changed=input.checked!==saved;
   if(changed)changes++;
   const label=$('[data-permission-label="'+input.value+'"]');
   label.textContent=changed?(input.checked?'待开放 · 未保存':'待取消开放 · 未保存'):(saved?'已对客户开放':'未对客户开放');
   label.classList.toggle('text-warning',changed);label.classList.toggle('text-secondary',!changed);
  });
  const hint=$('#template-permissions-status');
  if(!hint)return;
  hint.textContent=!selected.length?'请至少保留一款模板。':changes?
   '已勾选 '+selected.length+' 款 · '+changes+' 项待保存'+(!selected.includes(p.template)?'；当前模板已取消勾选，保存后将自动切换至剩余开放模板':''):
   '已开放 '+p.allowedTemplates.length+' 款 · 无待保存修改';
  $('#template-permissions-save').disabled=!selected.length||!changes;
  $('#template-permissions-reset').disabled=!changes;
 }
 ...
 function bindSettings(){
  ...
  if(isSuper&&permissions){
   ...
   permissions.addEventListener('submit',async e=>{
    e.preventDefault();
    const allowedTemplates=new FormData(permissions).getAll('allowedTemplates'),slug=state.project.slug;
    if(!allowedTemplates.length){toast('请至少开放一款模板',true);return;}
    const controls=[...permissions.querySelectorAll('input,button')];controls.forEach(el=>el.disabled=true);
    try{await post('/projects/'+slug,{allowedTemplates});toast('已更新该账号的开放模板');await detail(slug,true);}
    catch(error){toast(error.message,true);}
    finally{controls.forEach(el=>el.disabled=false);if(permissions.isConnected)syncTemplatePermissions();}
   });
   syncTemplatePermissions();
  }
  $('#settings-form [name=resetLogo]').addEventListener('change',e=>{const file=$('#settings-form [name=logo]');file.disabled=e.target.checked;if(e.target.checked)file.value='';});
  $('#settings-form').addEventListener('submit',e=>{e.preventDefault();const form=e.currentTarget;busy(form.querySelector('button'),async()=>{const fd=new FormData(form),data={appName:fd.get('appName'),downloadUrl:fd.get('downloadUrl')},file=fd.get('logo');if(isSuper){data.name=fd.get('name');data.note=fd.get('note');}if(fd.get('resetLogo'))data.logoData='';else if(file?.size){if(file.size>204800||!['image/png','image/jpeg','image/webp'].includes(file.type))throw Error('请选择 200 KB 以内的 PNG / JPG / WebP 图片');data.logoData=await new Promise((resolve,reject)=>{const r=new FileReader();r.onload=()=>resolve(r.result);r.onerror=()=>reject(Error('图片读取失败'));r.readAsDataURL(file);});}const result=await post(settingsPath(),data);state.project=result.project;toast('项目配置已保存');await detail(state.project.slug,true);});});
 }
