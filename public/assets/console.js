(() => {
 ...
 function renderRows(){const q=state.search.trim().toLowerCase(),rows=state.projects.filter(p=>(state.filter==='all'||p.status===state.filter)&&[p.name,p.slug,p.username].some(v=>v.toLowerCase().includes(q)));$('#project-rows').innerHTML=rows.map((p,i)=>`<tr><td><div class="d-flex align-items-center gap-3"><div class="avatar bg-primary-lt tone-${i%4}">${escape((p.app_name||p.name).slice(0,1).toUpperCase())}</div><div><strong>${escape(p.name)}</strong><small class="font-monospace d-block text-secondary">/p/${p.slug}</small></div></div></td><td><code>${escape(p.username)}</code><div><button class="btn btn-link btn-sm" data-action="credentials" data-slug="${p.slug}">查看密码</button>${p.credentialsAvailable?'':'<small class="text-secondary">旧版账号</small>'}</div></td><td>${status(p)}</td><td><span class="badge bg-blue-lt">${p.template==='dptv'?'DPTV':'Feiyue'}</span></td><td>${n(p.visits)} <span class="font-monospace">/ ${n(p.conversions)}</span></td><td>${date(p.created_at)}</td><td><button class="btn btn-link btn-sm" data-action="manage" data-slug="${p.slug}">管理</button><button class="btn btn-link btn-sm" data-action="copy-delivery" data-slug="${p.slug}">复制交付</button><button class="btn btn-link btn-sm text-secondary" data-action="copy-url" data-slug="${p.slug}">复制地址</button><button class="btn btn-link btn-sm text-danger" data-action="delete-project" data-slug="${p.slug}">删除</button></td></tr>`).join('')||'<tr><td colspan="7"><div class="empty text-secondary py-4"><span>▦</span><b>这里还没有匹配的项目</b><p>试试其他关键词，或点击右上角开通一个新项目。</p></div></td></tr>';$('#project-count').textContent='显示 '+rows.length+' 个项目 · 共 '+state.projects.length+' 个';}
