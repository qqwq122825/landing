(() => {
 ...
 const state={csrf:'',username:'',projects:[],totals:{},active:0,view:'overview',project:null,stats:null,page:1,days:7,search:'',filter:'all',credentials:null,deleteSlug:null,deleting:false,pendingTemplate:null,previewMode:null,templates:[],templateSearch:'',selectedTemplate:null,audit:[]};
 ...
 async function overview(){
  const [data,log]=await Promise.all([api('/projects'),api('/audit')]);Object.assign(state,{projects:data.projects,totals:data.totals,active:data.active,audit:log.rows});setView('overview','项目总览');
  $('#main').innerHTML=heading('项目总览','所有落地页，一个工作空间。管理客户、配置入口，了解每次访问。','<button class="btn btn-outline-secondary" data-action="reload">↻ 刷新</button><button class="btn btn-primary" data-action="create">＋ 开通项目</button>')+metrics(data.totals,true)+`<div class="alert alert-info hub-intro"><div><h3>从开户到上线，只需要一个入口。</h3><p>自动生成项目地址和客户账号，独立管理下载链接与访问数据。</p></div><div class="font-monospace">你的域名 <strong>/p/项目编号</strong> /admin</div></div><section class="card"><div class="card-header"><h2 class="card-title">全部项目 <span class="badge bg-primary-lt">${state.projects.length}</span></h2><div class="filters"><div class="input-icon"><input class="form-control" id="project-search" aria-label="搜索项目" placeholder="搜索项目名称、编号或账号" value="${escape(state.search)}"></div><select class="form-select" id="status-filter" aria-label="项目状态筛选"><option value="all">全部状态</option><option value="active">运行中</option><option value="paused">已暂停</option></select></div></div><div class="table-responsive"><table class="table table-vcenter table-hover"><thead><tr><th>项目 / 编号</th><th>客户账号 / 密码</th><th>状态</th><th>当前模板</th><th>访问 / 转化</th><th>创建时间</th><th>操作</th></tr></thead><tbody id="project-rows"></tbody></table></div><div class="card-footer d-flex justify-content-between align-items-center flex-wrap gap-2"><span id="project-count"></span><span>项目地址永久固定 · 账号独立分配</span></div></section><div class="bottom-grid"><section class="card"><div class="card-header"><h2 class="card-title">从这里，开始一个新项目</h2></div><div class="card-body"><ul class="steps steps-counter my-3"><li class="step-item active"><strong>开通项目</strong><p class="text-secondary small">命名并选模板</p></li><li class="step-item"><strong>交付账号</strong><p class="text-secondary small">复制登录信息</p></li><li class="step-item"><strong>独立运营</strong><p class="text-secondary small">客户独立配置</p></li></ul></div></section><section class="card"><div class="card-header"><h2 class="card-title">最近动态</h2><button class="btn btn-link btn-sm" data-view="audit">查看全部 →</button></div><div class="list-group list-group-flush">${recent(log.rows)}</div></section></div>`;
 ...
 function recent(rows){return rows.slice(0,3).map(r=>`<div class="list-group-item d-flex align-items-center justify-content-between gap-3"><span>${escape(r.action)}${r.project?' · '+escape(r.project):''}</span><small>${date(r.created_at)}</small></div>`).join('')||'<p class="form-hint">还没有操作记录。创建项目后会自动记录。</p>';}
 ...
 async function templateLibrary(){
  ...
  $('#main').innerHTML=heading('模板管理','所有模板集中查看，预览原始效果，了解每款模板的使用和开放情况。','<button class="btn btn-outline-secondary" data-action="reload">↻ 刷新</button>')+
   `<div class="alert alert-info d-flex align-items-center flex-wrap gap-3"><span><strong>${n(data.templates.length)}</strong> 款模板</span><span><strong>${n(data.projectCount)}</strong> 个项目</span><p>预览不创建项目、不切换模板，也不计入访问。</p></div><section class="card"><div class="card-header"><h2 class="card-title">全部模板 <span class="badge bg-primary-lt">${n(data.templates.length)}</span></h2><div class="input-icon"><input class="form-control" id="template-search" aria-label="搜索模板" placeholder="搜索模板名称或编号" value="${escape(state.templateSearch)}"></div></div><div id="template-library-grid" class="template-library-grid"></div><p id="template-library-count" class="card-footer text-secondary" role="status"></p></section><section id="template-projects" class="card" aria-label="模板关联项目"></section><p class="form-hint">使用数与开放数包含已暂停项目。开放权限仍按账号设置，进入关联项目的「管理」即可调整；这里的预览不修改任何客户配置。</p>`;
